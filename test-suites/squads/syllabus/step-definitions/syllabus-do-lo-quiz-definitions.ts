...
export async function studentSeesTopicProgress(
    ...
    if (studyPlanItemList.has(topicId)) {
        if (studyPlanItemList.get(topicId) != null) {
            numOfTotalTopicItems = studyPlanItemList.get(topicId)!.length;
        }
    }
...
export async function teacherSeesLatestQuizResultOnStudyPlanTable(
    ...
    const loProgressInStudyPlanTable = new ByValueKey(
        SyllabusTeacherKeys.studentStudyPlanItemGradeText(studyPlanItemName)
    );
    ...
    const loQuestions = scenario.get<string[]>(aliasQuizQuestionNames);
    const numOfLoQuestions = loQuestions.length;

    await weExpect(
        quizLatestResult,
        `Quiz latest result on Study Plan Table is updated to 1/${numOfLoQuestions}`
    ).toEqual(quizResult);
}
