...
Given(`student goes to Flashcard Learn screen`, async function (): Promise<void> {
...
Then(`student sees Flashcard Learn screen`, async function (): Promise<void> {
    const context = this.scenario;
    const learner = this.learner;
    const flashcardName = context.get<string>(aliasFlashcardName);
...
Then(
    `student does not see options button in Flashcard Learn screen`,
    async function (): Promise<void> {
        const learner = this.learner;

        await learner.instruction(`Student does not see options button`, async () => {
            await studentDoesNotSeeOptionsBottomButtonInFlashcardLearn(learner);
        });
    }
);
