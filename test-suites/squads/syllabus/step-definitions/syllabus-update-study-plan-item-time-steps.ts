...
export async function selectStudyPlanItemsByTopic(
    ...
    const selectedStudyPlanItemsByTopicId = new Map<string, string[]>();
    ...
    for (const topicName of topicNames) {
        ...
        await teacher.instruction(
            ...
            async function (this: TeacherInterface) {
                ...
                selectedStudyPlanItemsByTopicId.set(
                    topicId,
                    unselectedStudyPlanItemsByTopicId.get(topicId) ?? []
                );
                unselectedStudyPlanItemsByTopicId.delete(topicId);
            }
        );
    }
...
export async function selectStudyPlanItems(
    ...
    for (const key of selectedStudyPlanItemsByTopicId.keys()) {
        await teacher.instruction(
            `teacher scrolls into topic ${key}`,
            async function (this: TeacherInterface) {
                await teacherScrollIntoTopic(this, 'Topic ' + key);
            }
        );

        await teacher.instruction(
            `teacher selects items in topic ${key} randomly`,
            async function (this: TeacherInterface) {
                await teacherSelectsStudyPlanItems(this, selectedStudyPlanItemsByTopicId.get(key)!);
            }
        );
    }
