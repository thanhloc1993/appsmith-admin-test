...
import { studentRefreshHomeScreen } from './syllabus-utils';
...
Then(
    'school admin sees a new matched studyplan created for master studyplan on CMS',
    async function () {
...
Then(
    ...
    async function () {
        ...
        await asyncForEach<LearnerInterface, void>(learners, async (learner) => {
            ...
            await this.teacher.instruction(
                `Teacher sees study plan of ${profile.name}`,
                async () => {
...
Then('all students do not see new studyplan items on Learner App', async function () {
    ...
    await asyncForEach<LearnerInterface, void>(learners, async (learner) => {
        await learner.instruction('Student refreshes learner app', async () => {
            await studentRefreshHomeScreen(learner);
        });

        await learner.instruction('Student goes to the course student detail', async () => {
            await learner.flutterDriver!.tap(
                new ByValueKey(SyllabusLearnerKeys.course(courseName))
            );
        });

        await learner.instruction(
            `Student sees nothings in the course: ${courseName}`,
            async () => {
...
Then(
    'school admin sees a new matched studyplan created for all students studyplan on CMS',
    async function () {
        const studyPlanName = this.scenario.get<string>(aliasStudyPlanName);
        const learners: LearnerInterface[] = [this.learner, this.learner2];
        ...
        await this.cms.instruction('School admin chooses the student study plan tab', async () => {
        ...
        await asyncForEach<LearnerInterface, void>(learners, async (learner) => {
            ...
            if (!studyPlanItemList) {
                await this.cms.instruction('School admin sees an empty table', async () => {
            ...
            await this.cms.instruction(
                'School admin goes back to the course study plan tab',
                async () => {
