...
export class SyllabusLearnerKeys {
    ...
    static attemptHistoryRecordItem = (index: number) => `Attempt History Record Item ${index}`;
    ...
    static pointsPerQuestion = (points: number) => `${points} pts`;

    static quizFinishedAchievementScreen = (loName: string): string =>
        `${loName} Quiz Finished Achievement Screen`;
    ...
    static viewAnswerKeyButtonWithState = (isEnabled: boolean) =>
        `View Answers Keys Button${isEnabled ? 'Enabled' : ' Disabled'}`;
    ...
    static startExamLOButtonDisabled = 'Start Exam LO Button Disabled';
    ...
    static examLOSubmissionAttempt = 'Exam LO Submission Attempt';

    static examLOSubmissionAttemptWithValue(maximumAttempt: number) {
        return `Exam LO Submission Attempt ${maximumAttempt}`;
    }

    static examLOGradeToPass = 'Exam LO Grade To Pass';

    static examLOGradeToPassWithValue(gradeToPass: number) {
        return `Exam LO Grade To Pass ${gradeToPass}`;
    }
    ...
    static gradeBookStudyPlanName = (studyPlanName: string): string =>
        `Grade Book Study Plan Name ${studyPlanName}`;

    static gradeBookCourseName = (courseName: string): string =>
        `Grade Book Course Name ${courseName}`;

    static gradeBookCompletedRateByStudyplanName = (
        studyPlanName: string,
        completedRate: string
    ): string => `Grade Book Completed Rate By Study Plan Name ${studyPlanName} : ${completedRate}`;

    static gradeBookPassedRateByStudyPlanName = (
        studyPlanName: string,
        passedRate: string
    ): string => `Grade Book Passed Rate By Study Plan Name ${studyPlanName} : ${passedRate}`;

    static gradeBookScoreAndAttemptByLearningMaterialId = (
        learningMaterialId: string,
        score: string,
        attempt: string
    ): string =>
        `Grade Book Score And Attempt By Learning Material Id ${learningMaterialId} : ${score} ${attempt}`;

    static gradeBookEmptyScoreByLearningMaterialId = (learningMaterialId: string): string =>
        `Grade Book Empty Score By Learning Material Id ${learningMaterialId}`;

    static quizFinishedScreenConfiguration = `quizFinishedScreenConfiguration`;
}
