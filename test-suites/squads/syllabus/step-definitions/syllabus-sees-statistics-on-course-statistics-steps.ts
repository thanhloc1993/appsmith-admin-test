...
Given(
    ...
    async function (this: IMasterWorld) {
        ...
        const listLearningTypesWithScore = [
            // 'Assignment requires grading',  // another grading flow, will test in another test
            'Exam LO',
            'Flashcard',
            'LO with quizzes',
            'TaskAssignment has Correctness',
        ] as LearningTypeWithScore[];
        const listLearningTypesWithoutScore = [
            'non LO with quizzes',
            'Assignment non requires grading',
            'TaskAssignment no Correctness',
        ] as LearningTypeWithoutScore[];

        const selectedLearningTypeWithScore = getRandomElement<LearningTypeWithScore>(
            listLearningTypesWithScore
        ) as LearningType;
        const selectedLearningTypeWithoutScore = getRandomElement<LearningTypeWithoutScore>(
            listLearningTypesWithoutScore
        ) as LearningType;
        ...
        const taskAssignmentRequireCorrectnessName = scenario.get<string>(
            aliasTaskAssignmentWithCorrectnessName
        );
        const taskAssignmentNotRequireCorrectnessName =
            scenario.get<string>(aliasTaskAssignmentName);

        const assignmentRequireGradeName = scenario.get<string>(aliasAssignmentName);
        const assignmentNotRequireGradeName = scenario.get<string>(
            aliasAssignmentNotRequireGradeName
        );

        const loWithQuizName = scenario.get<string>(aliasLOName);
        const loNoQuizName = scenario.get<string>(aliasLOWithMaterialName);

        const examLOName = scenario.get<string>(aliasExamLOName);
        const flashcardName = scenario.get<string>(aliasFlashcardName);
        ...
        switch (selectedLearningTypeWithScore as LearningTypeWithScore) {
        ...
        switch (selectedLearningTypeWithoutScore as LearningTypeWithoutScore) {
...
When(
    ...
    async function (this: IMasterWorld) {
        ...
        for (const student of studentList) {
            ...
            await studentDoesStudyPlanItemNoScore(
                learner,
                topicName,
