...
import { genId, randomInteger } from '@legacy-step-definitions/utils';
...
import {
    schoolAdminSeesTimeLimitValidationErrorMessage,
    schoolAdminSeesTimeLimit,
    schoolAdminFillsExamWithTimeLimit,
    schoolAdminFillsTimeLimit,
    studentSeeTimeLimit,
...
Then('school admin sees exam detail with time limit on CMS', async function () {
    ...
    await this.cms.instruction(
        ...
        async () => {
            await schoolAdminSeesTimeLimit(
                this.cms,
...
Then(`school admin sees time limit's validation error message`, async function () {
    ...
    await this.cms.instruction(
        ...
        async () => {
            await schoolAdminSeesTimeLimitValidationErrorMessage(this.cms, message);
        }
    );
});
