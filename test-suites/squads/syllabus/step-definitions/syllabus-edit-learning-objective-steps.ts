...
When(
    `school admin edits LO {string}`,
    async function (this: IMasterWorld, loInfo: LoInfo): Promise<void> {
        const context = this.scenario;
        const loName = context.get<string>(aliasLOName);

        await this.cms.instruction(
            `school admin goes to LO details`,
            async function (this: CMSInterface) {
                const loName = context.get<string>(aliasLOName);
                await schoolAdminIsOnBookDetailsPage(this, context);
                const loRow = await this.page!.waitForSelector(loAndAssignmentByName(loName));
                await loRow.click();
                await this.assertThePageTitle(loName);
            }
        );
        ...
        await this.cms.instruction(`school admin updates the ${loInfo} of the LO`, async () => {
            switch (loInfo) {
                case 'name':
                    return schoolAdminUpdateLOName(this.cms, context, loName);
                case 'video': {
...
Then(
    `school admin sees the edited LO {string} on CMS`,
    async function (this: IMasterWorld, loInfo: LoInfo): Promise<void> {
        const context = this.scenario;

        await this.cms.instruction(
            `school admin sees the LO with updated ${loInfo}`,
            async function (this: CMSInterface) {
                await schoolAdminSeesUpdatedLoInfo(this, context, loInfo);
            }
        );
    }
);
...
Then(
    `student sees the edited LO {string} on Learner App`,
    async function (this: IMasterWorld, loInfo: LoInfo): Promise<void> {
        const context = this.scenario;
        ...
        await this.learner.instruction(
            `student goes to the topic details of the LO`,
            async function (this: LearnerInterface) {
        ...
        await this.learner.instruction(
            `student sees the LO with the update ${loInfo}`,
            async function (this: LearnerInterface) {
                await studentSeesUpdatedLoInfo(this, context, loInfo);
            }
        );
    }
);
...
Then(
    `teacher sees the edited LO {string} on Teacher App`,
    async function (this: IMasterWorld, loInfo: LoInfo): Promise<void> {
        const context = this.scenario;
        const courseName = context.get<string>(aliasCourseName);
        ...
        await this.teacher.instruction(
            `teacher sees the LO with the update ${loInfo}`,
            async function (this: TeacherInterface) {
                await teacherSeesUpdatedLoInfo(this, context, loInfo);
            }
        );
    }
);
