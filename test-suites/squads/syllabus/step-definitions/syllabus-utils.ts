...
export const studentTapNextButton = async (learner: LearnerInterface) => {
...
// TODO: it is cloned in exam, so we can remove when it is cloned in other cms-locators
...
export async function studentGoesToTodosScreen(learner: LearnerInterface) {
    ...
    try {
        await learner.flutterDriver?.waitFor(todoTab, 5000);
...
export async function studentRefreshToDoScreen(learner: LearnerInterface, toDoTabPage: string) {
    await learner.flutterDriver!.waitFor(new ByValueKey(toDoTabPage), 10000);
    //scroll to top
...
export function searchingStudyPlanItemNamesByTopicId(
    selectedStudyPlanItems: SelectedStudyPlanItem[],
    topicId: string
): string[] {
    const result: string[] = [];
    for (const selectedStudyPlanItem of selectedStudyPlanItems) {
        if (selectedStudyPlanItem.topicId.includes(topicId)) {
            return selectedStudyPlanItem.studyPlanItemNames;
        }
    }
    return result;
}
...
export function convertToStudyPlanItemNamesByTopicId(
    ...
    const studyPlanItemsByTopicIds: SelectedStudyPlanItem[] = [];
    for (const studyPlanItem of studyPlanItems) {
        ...
        if (topicId != undefined) {
            const studyPlanItemsByTopicId = searchingStudyPlanItemNamesByTopicId(
                studyPlanItemsByTopicIds,
                topicId
            );
            if (studyPlanItemsByTopicId.length == 0) {
                studyPlanItemsByTopicIds.push({
                    topicId: topicId,
                    studyPlanItemNames: [studyPlanItemName],
                });
            } else {
...
export function getQuizTypeValue({
    ...
    switch (quizTypeTitle || quizTypeNumber || quizTypeKey) {
        ...
        case 'ordering':
        case QuizType.QUIZ_TYPE_ORD:
        case KeyQuizType.QUIZ_TYPE_ORD:
            return {
                quizTypeKey: 'QUIZ_TYPE_ORD',
                quizTypeNumber: QuizType.QUIZ_TYPE_ORD,
                quizTypeTitle: 'ordering',
            };
        default:
            return {
...
export function getConvertedStringType<T>(rawString: string): T {
    if (rawString.startsWith('1 of [')) {
        return randomOneOfStringType<T>(rawString);
    }
...
export const teacherWaitingStudentStudyPlanPage = async (teacher: TeacherInterface) => {
    const teacherDriver = teacher.flutterDriver!;
    const studyPlanScreenKey = new ByValueKey(SyllabusTeacherKeys.studentStudyPlanScreen);
    const studyPlanTabKey = new ByValueKey(SyllabusTeacherKeys.studentStudyPlanTab);
    const studyPlanItemListKey = new ByValueKey(SyllabusTeacherKeys.studentStudyPlanItemList);
    const studyPlanScrollViewKey = new ByValueKey(SyllabusTeacherKeys.studentStudyPlanScrollView);

    await teacherDriver.waitFor(studyPlanScreenKey);
    await teacherDriver.waitFor(studyPlanTabKey);
    await teacherDriver.waitFor(studyPlanItemListKey);
    await teacherDriver.waitFor(studyPlanScrollViewKey);
};
