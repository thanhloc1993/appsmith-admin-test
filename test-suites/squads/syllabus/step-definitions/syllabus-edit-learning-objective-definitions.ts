...
export async function schoolAdminSelectEditLO(cms: CMSInterface, loName: string) {
    await cms.selectActionButton(ActionOptions.EDIT, { target: 'actionPanelTrigger' });
    const loInputValue = await cms.page!.inputValue(`input${loInputSelector}`);
    weExpect(loInputValue).toEqual(loName);
}

export async function schoolAdminUpdateLOName(cms: CMSInterface, loName: string) {
    const loUpdatedName = `Updated ${loName}`;
    await cms.page!.fill(`input${loInputSelector}`, loUpdatedName);
    await cms.selectAButtonByAriaLabel(`Save`);
    await cms.assertThePageTitle(loUpdatedName);

    return { loName: loUpdatedName };
}

export async function schoolAdminUploadLOStudyGuide(cms: CMSInterface) {
    await cms.uploadAttachmentFiles(samplePDFFilePath, FileTypes.PDF);
}
...
export async function schoolAdminUploadLOVideoLink(cms: CMSInterface) {
    await cms.page!.fill(assignmentBrightcoveUploadInput, brightCoveSampleLink);
    await cms.selectAButtonByAriaLabel(`Upload`);
...
export async function schoolAdminUploadLOVideoLinkWithLink(cms: CMSInterface, link: string) {
    await cms.instruction(
...
export const schoolAdminUploadLOVideoFile = async (cms: CMSInterface) => {
    ...
    await cms.uploadAttachmentFiles(sampleVideoFilePath, FileTypes.VIDEO);
...
export async function schoolAdminSeesUpdatedLoInfo(
    ...
    switch (loInfo) {
        case 'name': {
            const loName = context.get(aliasLOName);
