...
When(`student selects Test Button in Flashcard Detail Screen`, async function (): Promise<void> {
    const learner = this.learner;

    await learner.instruction(`Student tap test button`, async () => {
        await studentSelectsTestButtonInFlashcardDetailPage(learner);
    });
});
