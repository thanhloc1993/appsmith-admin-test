...
import {
    schoolAdminAssertGradeToPass,
    schoolAdminAssertGradeToPassErrorMsg,
    schoolAdminFillExamWithGradeToPass,
} from 'test-suites/squads/syllabus/step-definitions/create-exam-with-grade-to-pass-definitions';
...
Then('school admin sees exam detail with grade to pass in CMS', async function () {
    ...
    await this.cms.instruction(
        ...
        async () => {
            await schoolAdminAssertGradeToPass(
                this.cms,
...
Then('school admin sees error message under grade to pass input field', async function () {
    ...
    await this.cms.instruction(`School admin sees error message: ${message}`, async () => {
        await schoolAdminAssertGradeToPassErrorMsg(this.cms, message);
    });
});
