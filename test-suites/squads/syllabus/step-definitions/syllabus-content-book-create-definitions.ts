...
import { quizModifierService } from '@supports/services/eureka/quiz';
...
import { schoolAdminShouldUseInsertAssignment } from './syllabus-migration-temp';
import {
    getLOTypeValue,
    getQuizTypeValue,
...
export async function schoolAdminIsOnBookDetailsPage(cms: CMSInterface, context: ScenarioContext) {
    ...
    const bookName = context.get<string>(aliasBookName);
...
export async function schoolAdminHasCreatedAChapterTopicOnlyBook(
    cms: CMSInterface,
    scenario: ScenarioContext
) {
    await cms.instruction(
        ...
        async function (this: CMSInterface) {
            ...
            } = await createRandomChapters(this, { bookId: books[0].bookId });
            ...
            const {
                request: { topicsList },
            } = await createRandomTopics(this, {
                quantity: 1,
                chapterId: chaptersList[0].info!.id,
            });
            scenario.set(aliasTopicName, topicsList[0].name);
            scenario.set(
                aliasTopicNames,
                topicsList.map((topic) => topic.name)
            );
        }
    );
}
