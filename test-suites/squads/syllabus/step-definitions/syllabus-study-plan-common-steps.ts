...
import { randomUniqueIntegers } from '@legacy-step-definitions/utils';
...
import {
    ...
    schoolAdminGoesToCourseViaBreadcrumbs,
    schoolAdminGoesToStudentStudyPlanDetailPage,
    schoolAdminGoesToStudyPlanTabFromCourse,
...
Given(
    ...
    async function (
        ...
        this.scenario.set(aliasStudyPlanName, studyPlanList[0].name);
    }
);
...
Then(
    ...
    async function (this: IMasterWorld, studentRole: AccountRoles) {
        ...
        await this.cms.instruction(
            `School admin goes to individual study plan tab on course ${courseName} detail page`,
            async () => {
                await schoolAdminGoesToCourseViaBreadcrumbs(this.cms, courseName);
                await schoolAdminGoesToStudyPlanTabFromCourse(this.cms, 'individual');
            }
        );
