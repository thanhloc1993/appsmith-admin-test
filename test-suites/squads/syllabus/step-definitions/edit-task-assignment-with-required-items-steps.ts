...
import {
    ...
    schoolAdminSeeTaskAssignmentRequiredItem,
    schoolAdminSeeTaskAssignmentRequiredItemsIsEmpty,
    ...
    async function (this: IMasterWorld, type: EditType) {
        ...
        switch (type) {
            ...
            case 'not include required items': {
                ...
                this.scenario.set(aliasTaskAssignmentSetting, []);
                break;
            }
        }
    }
...
Then("school admin sees task assignment's required items edited", async function () {
    const settings = this.scenario.get<TaskAssignmentSettingInfo[]>(aliasTaskAssignmentSetting);

    const isEmptySetting = settings.length === 0;

    if (isEmptySetting) {
        await this.cms.instruction(
            "school admin sees the task assignment's required items is empty",
            async () => {
                await schoolAdminSeeTaskAssignmentRequiredItemsIsEmpty(this.cms);
            }
        );
    ...
    await this.cms.instruction(
        "school admin sees the task assignment's required items edited" + JSON.stringify(settings),
        async () => {
            await asyncForEach(settings, async (setting) => {
                await schoolAdminSeeTaskAssignmentRequiredItem(this.cms, setting);
            });
        }
    );
});
