...
import { asyncForEach, randomInteger } from '@legacy-step-definitions/utils';
...
import { examLOAddQuestionButton } from './cms-selectors/exam-lo';
import { quizTableRoot } from './cms-selectors/syllabus';
import {
    getOCRTypeSelector,
...
export const schoolAdminChooseAddQuestionButtonByLOType = async (
    ...
    if (isGroupOfQuestionEnabled) {
        ...
        if (loType === 'exam LO') {
            return await cmsPage.click(examLOAddQuestionButton);
        }
...
export const schoolAdminFillsAllAnswersByQuestionType = async (
    ...
    if (questionType === 'manual input') return answerContentList;
