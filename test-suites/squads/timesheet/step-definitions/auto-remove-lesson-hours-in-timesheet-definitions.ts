...
export async function goToStaffTimesheetSettingTab(cms: CMSInterface, staff: UserProfileEntity) {
    await cms.instruction(
        `School admin goes to timesheet setting tab for staff ${staff.name}`,
        async function () {
...
export async function addStudentFromOneLocationToLesson(cms: CMSInterface, locationName: string) {
    ...
    await page.click(withNth(openFilterAdvancedPopupButton, 2));
