...
export const assertElementDoesNotExist = async (cms: CMSInterface, selector: string) => {
    await assertElementCountToBe(cms, selector, 0);
};
...
export const assertElementCountToBe = async (
    cms: CMSInterface,
    selector: string,
    expectedCount: number
) => {
    const actualCount = await cms.page!.locator(selector).count();
    weExpect(actualCount).toBe(expectedCount);
};
