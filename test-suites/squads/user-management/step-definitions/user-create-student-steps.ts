...
// school admin has created students "student S1, student S2" belong to location
Given(
    'school admin has created students {string} belong to location',
    async function (roles: string) {
        ...
        const studentRoles = roles.split(',') as AccountRoles[];

        const studentList: AccountRoles[] = context.get<AccountRoles[]>(studentListAlias) ?? [];

        await cms.attach(`Create ${studentList.length} student(s) with location by GRPC`);

        await Promise.all(
            studentRoles.map(async (studentRole) => {
                ...
                studentList.push(studentRole);

                context.set(studentRole, studentData.student);
                context.set(
                ...
                if (!locations || !locations.length) {
                    throw Error('No location found, cannot create student with location');
                }
