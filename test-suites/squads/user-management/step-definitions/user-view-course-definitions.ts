...
export async function viewCourseOnLearnerApp(
    ...
    const courseFinder = new ByValueKey(SyllabusLearnerKeys.course(courseName));
    ...
    if (courseAvatar !== undefined && courseAvatar !== null) {
        await learner.instruction(`Verify course avatar ${courseAvatar} `, async function () {
            const courseAvatarFinder = new ByValueKey(
                SyllabusLearnerKeys.courseAvatar(courseAvatar)
            );
    ...
    await learner.instruction(
        ...
        async function () {
            const appBarFinder = new ByValueKey(LearnerKeys.appBar);
