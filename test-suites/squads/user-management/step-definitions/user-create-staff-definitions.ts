...
import { applyOrgForLocationSetting, clickOnSaveInDialog } from './user-definition-utils';
import { getUserGroupList, getUserGroupsWithTeacherRole } from './user-query-user-groups-hasura';
...
export async function schoolAdminCreateAStaff({
    ...
    await cms.instruction('Initialize and select user group', async function (this: CMSInterface) {
        ...
        const userGroupField = page.locator(staffSelector.staffFormUserGroupInput);
        await userGroupField.click();
        const userGroupInput = userGroupField.locator(autoCompleteBaseInput);
...
export async function createRandomStaffByGRPC(
    ...
    const userGroupList = await getUserGroupList(cms);
    ...
    await usermgmtStaffModifierService.createStaff(token, staffReq);
    return {
        name: staffReq.name,
