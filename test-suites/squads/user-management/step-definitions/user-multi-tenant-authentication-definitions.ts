...
export async function schoolAdminVerifiesStaffListWithResourcePath(cms: CMSInterface) {
    await cms.schoolAdminIsOnThePage(Menu.STAFF, 'Staff Management');
    await cms.waitForSkeletonLoading();

    let userList: StaffTypes[];

    await cms.instruction('School admin change the rows per page into 50', async function () {
        const [staffListResponse] = await Promise.all([
            cms.waitForHasuraResponse(UserHasuraQueryNames.STAFF_LIST_QUERY),
            changeRowsPerPage(cms, 50),
        ]);
        userList = staffListResponse.resp.data.users?.map((item: StaffListHasuraResponseType) => ({
    ...
    await cms.instruction(`School admin verifies staff list with resource_path`, async function () {
        const { schoolId } = await cms.getProfile();
        ...
        for (let index = 0; index < userList.length; index++) {
            ...
            weExpect(
                schoolId.toString(),
                `organization's resource path should equal to staff's resource path: 
                    staff's ID: ${user.user_id} 
                    staff's resource path: ${user.resource_path} 
                    organization's resource path: ${schoolId}`
            ).toBe(user.resource_path);
        }
    });
