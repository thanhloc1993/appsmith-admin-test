...
export async function fillInParentInformation(
    ...
    const isEnableFirstLastNameParent = await isEnabledFeatureFlag(
        'USER_PARENT_ENABLE_LAST_FIRST_PHONETIC_NAME'
    );
    ...
    if (isEnableFirstLastNameParent) {
        await addParentDialog.getByLabel('Last Name *').fill(splittedName[0]);
        await addParentDialog.getByLabel('First Name *').fill(splittedName[1]);
    } else {
    ...
    const parentPrimaryPhoneNumberTextField = addParentDialog.getByLabel(
        'Parent Primary Phone Number'
    );
    await parentPrimaryPhoneNumberTextField.fill(phoneNumber);

    return {
        name: isEnableFirstLastNameParent ? `${splittedName[0]} ${splittedName[1]}` : username,
        email: username,
        phoneNumber,
...
export async function clickOnAddParentAndFillInParentInformation(
    ...
    await cms.instruction(`Fill in parent information: ${parentUsername}`, async function () {
        const { name } = await fillInParentInformation(cms, parentUsername, parentPhoneNumber);
        parentName = name;
    });
