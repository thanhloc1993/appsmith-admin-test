...
import {
    ...
    createRandomStaffByGRPCAndReissuePassword,
    ...
    async function (this: IMasterWorld, condition: UpsertStaffConditionType) {
        ...
        switch (condition) {
            case 'only mandatory inputs':
                ...
                staffFormData.firstNamePhonetic = '';
                staffFormData.lastNamePhonetic = '';
                staffFormData.fullNamePhonetic = '';
                break;
            case 'empty name':
                staffFormData.name = '';
...
Given('school admin has created a staff', async function (this: IMasterWorld) {
    ...
    await this.cms.instruction(
        ...
        async function (this: CMSInterface) {
            const createdStaff = await createRandomStaffByGRPCAndReissuePassword(cms);
            context.set(staffProfileAlias, { ...createdStaff, id: createdStaff.staffId });
        }
    );
});
