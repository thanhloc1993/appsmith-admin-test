...
import bankBranchBobQuery from '@supports/graphql/bob/bank-branch.query';
import bankBobQuery from '@supports/graphql/bob/bank.query';
import {
...
export async function getBankList(cms: CMSInterface) {
    const bankResponse = await cms.graphqlClient?.callGqlBob<User_Eibanam_GetListBankQuery>({
...
export async function getBankBranchList(cms: CMSInterface) {
    const bankBranchResponse =
        await cms.graphqlClient?.callGqlBob<User_Eibanam_GetListBankBranchQuery>({
