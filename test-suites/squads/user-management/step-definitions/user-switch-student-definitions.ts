...
export async function checkStudentAvatarOnAppBar(
    ...
    const studentAvatarFinder = new ByValueKey(
        SyllabusLearnerKeys.studentCurrentChildAvatar(learner.id)
    );
...
export async function parentSeesKidAtIndex(
    ...
    const studentTileFinder = new ByValueKey(
        SyllabusLearnerKeys.switchStudentKidTile(studentId, index)
    );
...
export async function parentSeesKidWithCorrectAvatar(
    ...
    const studentAvatarFinder = new ByValueKey(
        SyllabusLearnerKeys.kidDefaultAvatar(kid.id, studentFirstNameInitial)
    );
...
export async function userSeesNoStudentAssociated(parent: LearnerInterface) {
    const driver = parent.flutterDriver!;
    const noStudentsAssociatedFinder = new ByValueKey(SyllabusLearnerKeys.noStudentsAssociated);
