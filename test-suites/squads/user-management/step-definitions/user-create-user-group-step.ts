...
import { createUserGroupsWithGrantedRoles } from './user-create-user-group-with-draft-granted-permissions-definitions';
...
Given(
    ...
    async function (this: IMasterWorld, rolesTable: DataTable) {
        ...
        await createUserGroupsWithGrantedRoles(cms, context, grantedRoles);
    }
);

Given(
    'school admin has created a user group with role {string}',
    async function (this: IMasterWorld, grantedRole: UserRole) {
        const cms = this.cms!;
        const context = this.scenario;

        const grantedRoles: GrantedRoleTable[] = [
            {
                role: grantedRole,
                location: 'location A',
            },
