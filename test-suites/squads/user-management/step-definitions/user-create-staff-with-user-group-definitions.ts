...
import { forgotPasswordStaff, getNewPassword } from './user-create-staff-definitions';
import {
    clickMultipleTimes,
    ...
    await cms.instruction(
        ...
        async function () {
            ...
            if (isEnabledStaffPhoneticName) {
                await cmsPage.fill(staffFormLastNameInput, lastName);
                await cmsPage.fill(staffFormFirstNameInput, firstName);
            } else {
            ...
            const grantedPermissionsLocations = userGroups?.[0].grantedPermissions?.[0].locations;
...
export async function assignStaffIdAndPasswordToStaffProfile(
    ...
    await getNewPassword(cms, staffId!, staffProfile.password);
}
