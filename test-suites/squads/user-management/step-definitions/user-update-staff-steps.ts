...
import { getStaffInfo } from './user-forgot-password-definitions';
...
When('school admin edits staff with draft information', async function (this: IMasterWorld) {
    ...
    await this.cms.instruction(
        ...
        async function (cms: CMSInterface) {
            ...
            const firstName = `e2e-staff-edited`;
            const lastName = `e2e-staff-edited-${getRandomNumber()}`;
...
When('school admin edits staff name as empty', async function (this: IMasterWorld) {
    ...
    await this.cms.instruction(
        ...
        async function (cms: CMSInterface) {
            ...
            if (isEnabledStaffPhoneticName) {
                await cms.page?.fill(staffFormLastNameInput, '');
                await cms.page?.fill(staffFormFirstNameInput, '');
            } else {
...
Given('school admin has edited a staff email', async function (this: IMasterWorld) {
    ...
    const staff = context.get<UserProfileEntity>(staffProfileAlias);
    ...
    async function (this: IMasterWorld, condition: UpsertStaffConditionType) {
        ...
        switch (condition) {
            ...
            case 'all valid inputs':
                ...
                editStaffOptions.fillUserGroup = true;
                break;
            case 'empty name':
                newStaff.name = '';
                newStaff.firstName = '';
