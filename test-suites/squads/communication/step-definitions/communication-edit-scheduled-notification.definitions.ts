...
export async function editScheduleNotificationFields(
    ...
    switch (fieldKey) {
        case 'Title':
            ...
            context.set(aliasCreatedNotificationName, notificationEditTitle);
            break;
        ...
        case 'Content':
            ...
            context.set(aliasCreatedNotificationContent, notificationEditContent);
            break;
...
export async function assertEditedScheduleNotification(
    ...
    const notificationDetailContentElement = await cms.page!.waitForSelector(
        notificationDetailContent
    );

    await notificationDetailContentElement.scrollIntoViewIfNeeded();
    ...
    if (fieldKey === 'Individual Recipient')
        await cms.page?.waitForSelector(
            `${studentsAutocompleteHF} [role="button"][aria-label="${student2.name}"]`
        );
