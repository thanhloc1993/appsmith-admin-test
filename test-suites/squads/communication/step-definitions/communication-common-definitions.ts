...
export async function selectCoursesOnDialog(
    ...
    if (isAll) {
        await cms.instruction('Select all courses on compose dialog', async function () {
...
export function getCreatedNotificationID(scenario: ScenarioContext, notificationType: ComposeType) {
    const newNotificationID = scenario.get<string>(
        checkStatusScheduleNotification(notificationType)
            ? aliasCreatedScheduleNotificationID
            : aliasCreatedNotificationID
    );
