...
export async function cmsUpdateTeacherName(
    ...
    const firstName = teacherProfile.name;
    const lastName = `${teacherProfile.name}-edited`;
    const name = `${lastName} ${firstName}`;

    if (isEnabledStaffPhoneticName) {
        await page.fill(staffFormLastNameInput, lastName);
        await page.fill(staffFormFirstNameInput, firstName);
    } else {
        await page.fill(staffFormNameInput, name);
    }
