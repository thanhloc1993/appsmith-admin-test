...
export async function navigateToMaterialPageOnLearnerApp(
    learner: LearnerInterface,
    lessonId: string
) {
    const driver = learner.flutterDriver!;
...
export async function teacherTapPollDetailTabBarIfNeeded(teacher: TeacherInterface) {
    const driver = teacher.flutterDriver!;
    const tabBar = new ByValueKey(LessonKeys.pollingDetailTab(false));
    try {
        await driver.tap(tabBar);
    } catch {
        return;
    }
}

export async function teacherTapPollStatsTabBarIfNeeded(teacher: TeacherInterface) {
    const driver = teacher.flutterDriver!;
    const tabBar = new ByValueKey(LessonKeys.pollingStatsTab(false));
    try {
        await driver.tap(tabBar);
    } catch {
        return;
