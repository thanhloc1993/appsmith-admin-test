...
export async function teacherSeesWhiteFrameVisibleCoveredUserCameraStreamInTheMainScreenOnTeacherApp(
    ...
    await driver.waitFor(
        new ByValueKey(TeacherKeys.cameraDisplayContainerKey(userId, visible)),
        10000
    );
}
...
export async function isUserCameraVisibilityOnMainScreenOnTeacherApp(
    ...
    switch (cameraStatus) {
        case 'active':
            try {
                await driver.waitFor(
                    new ByValueKey(TeacherKeys.pinnedUserView(userId, true)),
                    10000
                );
                return true;
            } catch {
        ...
        case 'inactive':
            try {
                await driver.waitFor(
                    new ByValueKey(TeacherKeys.pinnedUserView(userId, false)),
                    10000
                );
                return true;
            } catch {
