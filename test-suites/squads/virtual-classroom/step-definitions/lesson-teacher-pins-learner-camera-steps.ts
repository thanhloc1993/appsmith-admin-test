...
import {
    ...
    userCameraHasPinnedOnTeacherApp,
...
Then(
    `{string} sees {string} stream in the main screen on Teacher App`,
    async function (teacherRole: AccountRoles, userRole: AccountRoles) {
        const teacher = getTeacherInterfaceFromRole(this, teacherRole);
        const userId = getUserIdFromRole(this.scenario, userRole);

        await teacher.instruction(`check ${userRole} camera has been pinned`, async function () {
            await userCameraHasPinnedOnTeacherApp(teacher, userId, true);
        });
    }
);
