...
import { getMaterialIdByEnv } from 'test-suites/squads/lesson/utils/utils';
...
export async function assertMaterialInLesson(params: {
    cms: CMSInterface;
    materials: MaterialFile[];
    state: MaterialFileState;
    org: 'MANABIE' | 'JPREP';
}) {
    const { cms, materials, state, org } = params;
    const page = cms.page!;

    if (org === 'MANABIE') {
        await page.locator(lessonDetailMaterialInfo).scrollIntoViewIfNeeded();
    }
...
export function generateMaterialWithType(materials: string): Material.AsObject[] {
    ...
    materialsType.forEach((materialType) => {
        ...
        if (isUploadedPDF) {
            materialsFile.push({
                mediaId: getMaterialIdByEnv('pdf'), //  pdf file
            });
        }
        if (isUploadedVideo) {
            materialsFile.push({
                mediaId: getMaterialIdByEnv('video'), // video file
            });
        }
    });
