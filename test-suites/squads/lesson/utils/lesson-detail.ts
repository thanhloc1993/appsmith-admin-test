...
export async function goToLessonDetailByLink(params: { cms: CMSInterface; lessonId: string }) {
    ...
    await cms.instruction(`Go to ${linkToLesson}`, async function () {
        await Promise.all([
            cms.waitForHasuraResponse('Lesson_SchedulerBySchedulerId'),
            cms.waitForHasuraResponse('Lesson_ClassByClassIdForLessonManagement'),
            cms.waitingForLoadingIcon(),
            cms.waitForSkeletonLoading(),
            cms.page!.goto(linkToLesson),
        ]);
    });
}
...
export async function learnerChooseLessonTab(learner: LearnerInterface) {
    ...
    const lessonTab = new ByValueKey(LearnerKeys.lessonTab);
    await flutterDriver.tap(lessonTab);
}
...
export async function assertLearnerSeeLesson(params: {
    ...
    const lessonItem = new ByValueKey(VirtualClassroomKeys.liveLessonItem(lessonId, ''));
    if (state === 'see') await flutterDriver.waitFor(lessonItem, 20000);
    else await flutterDriver.waitForAbsent(lessonItem, 10000);
}

export async function assertStudentInLessonUpsertByName(params: {
    cms: CMSInterface;
    studentName: string;
}) {
    const { cms, studentName } = params;

    const studentOnTableLocator = cms.page!.locator(checkBoxStudentInfoTable);
    await studentOnTableLocator.scrollIntoViewIfNeeded();
    const studentOnTable = await studentOnTableLocator.textContent();

    weExpect(studentOnTable, `Expect the newly added student has name ${studentName}`).toEqual(
        studentName
    );
}

export async function assertStudentInLessonDetailByName(params: {
    cms: CMSInterface;
    studentName: string;
}) {
    const { cms, studentName } = params;

    const studentLocator = cms.page!.locator(lessonInfoStudentNameColumn, { hasText: studentName });
    await studentLocator.scrollIntoViewIfNeeded();

    const isVisibleStudent = await studentLocator.isVisible();

    weExpect(isVisibleStudent, `Expect see student ${studentName}`).toEqual(true);
}
