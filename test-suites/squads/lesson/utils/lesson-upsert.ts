...
import { tableStudentAddAction } from '@legacy-step-definitions/cms-selectors/lesson-management';
...
import { convertOneOfStringTypeToArray, randomInteger } from '@legacy-step-definitions/utils';
import {
    learnerProfileAlias,
...
import {
    ...
    addStudentDialogTitle,
    addReallocateStudentDialogTitle,
    addNormalStudentOption,
    addReallocateStudentOption,
    selectTeachingMediumInput,
    getSelectTeachingMediumOption,
...
import { LessonFeatures } from 'test-suites/squads/lesson/common/enums';
...
export const parseTeachingMethodObject: Record<TeachingMethodValueType, TeachingMethod> = {
    Group: 'LESSON_TEACHING_METHOD_GROUP',
    Individual: 'LESSON_TEACHING_METHOD_INDIVIDUAL',
};

export async function selectTeachingMedium(cms: CMSInterface, teachingMedium?: TeachingMedium) {
    const defaultTeachingMedium: TeachingMedium = 'LESSON_TEACHING_MEDIUM_ONLINE';
    const isEnabledAttachZoomLink = await featureFlagsHelper.isEnabled(
        LessonFeatures.LESSON_MANAGEMENT_ATTACH_ZOOM_LINK
    );
    if (isEnabledAttachZoomLink) {
        await cms.page!.click(selectTeachingMediumInput);
        const desireTeachingMedium = getSelectTeachingMediumOption(
            teachingMedium || defaultTeachingMedium
        );

        await cms.page!.click(desireTeachingMedium);
    } else {
        const desireTeachingMedium = teachingMediumRadioButton(
            teachingMedium || defaultTeachingMedium
        );
...
export async function selectClassByNameWithoutAutoAllocate(cms: CMSInterface, className: string) {
...
export async function selectClassByNameV3(cms: CMSInterface, className: string) {
    ...
    await Promise.all([
        ...
        cms.chooseOptionInAutoCompleteBoxByText(className),
    ]);
}
...
export async function selectClassByNameV3GroupLesson(cms: CMSInterface, className: string) {
    ...
    await Promise.all([
        waitRetrieveStudentSubscriptionResponse(cms),
        cms.page!.click(footerDialogConfirmButtonSave),
    ]);
}
...
export async function fillUpsertFormLessonV3WithMissingFields(params: {
    ...
    if (areMissingFieldsNotEqualToCertainField(missingFields, 'location') && !!centerName) {
...
export async function fillUpsertFormLessonV3Updated(params: {
    ...
    if (
        areUpdatedFieldEqualToCertainField(updatedField, 'location') ||
    ...
    if (areUpdatedFieldEqualToCertainField(updatedField, 'location')) {
        const centerName = scenarioContext.get(aliasLocationName);
        await selectCenterByNameV3(cms, centerName);
        await acceptConfirmDialogIfExist(cms);
    }
...
export async function setupRecurringSetting(params: {
    cms: CMSInterface;
    scenarioContext: ScenarioContext;
    methodSaving: MethodSavingType;
}) {
    const { cms, scenarioContext, methodSaving } = params;

    await cms.instruction(`Select Recurring Setting: ${methodSaving}`, async function () {
        const setting: LessonSavingMethodType =
            methodSaving === 'One Time'
                ? 'CREATE_LESSON_SAVING_METHOD_ONE_TIME'
                : 'CREATE_LESSON_SAVING_METHOD_RECURRENCE';
        await selectRecurring(cms, setting);
    });
    if (methodSaving === 'Weekly Recurring') {
        ...
        await cms.instruction('Selects end date is lesson date of next month', async function () {
...
export async function createLessonByUI(params: {
    ...
    if (recurringSetting) {
        await setupRecurringSetting({ cms, scenarioContext, methodSaving: recurringSetting });
    }
...
export async function assertUpdatedIndividualLessonFieldOfOtherLessonInChain(
    ...
    if (field === 'location' || field === 'student') {
...
export async function assertUpdatedGroupLessonFieldOfOtherLessonInChain(
    ...
    if (field === 'location' || field === 'course' || field === 'class' || field === 'student') {
        studentName = users[1].name;
    }
...
export async function assertGroupLessonUpsertFieldUpdated(
    ...
    switch (field) {
        ...
        case 'class': {
            const className = scenarioContext.get(aliasClassName);
            const classNameContent = await cms.page!.locator(lessonInfoClassNames).textContent();

            weExpect(
                classNameContent?.includes(className),
...
export async function assertOtherIndividualLessonInChainNoChanged(
    ...
    const studentNames = await cms.page!.locator(lessonInfoStudentNameColumn).allTextContents();
    ...
    weExpect(studentNames, `Student names is ${lessonInfo.studentNames}`).toEqual(
...
export async function saveLessonWithStatus(
    cms: CMSInterface,
    lessonActionSave: LessonActionSaveType
) {
    switch (lessonActionSave) {
        case 'Draft':
            await cms.page!.click(lessonManagementLessonSubmitDraftButton);
            break;
        case 'Published':
...
export const lessonUpsertFields: LessonUpsertFields[] = [
    'lesson date',
    'end date',
    'start time',
    'teaching medium',
...
function isLessonUpsertField(fieldName: string): fieldName is LessonUpsertFields {
    return lessonUpsertFields.includes(fieldName as LessonUpsertFields);
}

export function convertToLessonUpsertFields(rawLessonUpsertFields: string): LessonUpsertFields[] {
    if (rawLessonUpsertFields === 'none') return [];

    const splittedItems = rawLessonUpsertFields.split(', ');

    return splittedItems.reduce((fields: LessonUpsertFields[], item) => {
        if (isLessonUpsertField(item)) fields.push(item);
        return fields;
    }, []);
}
...
export async function openAddStudentInfoDialog(params: {
    cms: CMSInterface;
    type: 'STANDARD' | 'REALLOCATE';
}) {
    const { cms, type } = params;
    const page = cms.page!;

    const isEnabledReallocate = await featureFlagsHelper.isEnabled(
        LessonFeatures.LESSON_MANAGEMENT_REALLOCATE_STUDENTS
    );

    if (!isEnabledReallocate) {
        await page.click(tableStudentAddAction);
        return;
    }

    await page.click(addStudentDropdownButton);

    switch (type) {
        case 'STANDARD':
            await page.click(addNormalStudentOption);
            await page.waitForSelector(addStudentDialogTitle);
            break;

        case 'REALLOCATE':
            await page.click(addReallocateStudentOption);
            await page.waitForSelector(addReallocateStudentDialogTitle);
            break;
    }
}

export function saveFilledLessonUpsertFields(params: {
    scenarioContext: ScenarioContext;
    lessonField: LessonUpsertFields | LessonUpsertFields[];
}) {
    const { scenarioContext, lessonField } = params;

    const currentFields = scenarioContext.get<LessonUpsertFields[]>(aliasFilledLessonFields) || [];
    const justFilledFields = Array.isArray(lessonField) ? lessonField : [lessonField];

    scenarioContext.set(aliasFilledLessonFields, [...currentFields, ...justFilledFields]);
}

export async function selectLessonDateByLessonTime(params: {
    cms: CMSInterface;
    scenarioContext: ScenarioContext;
    lessonTime: LessonTimeValueType;
}) {
    ...
    const newLessonDate = await changeDatePickerByDateRange({
        cms,
        currentDate,
        datePickerSelector: lessonDateV3,
        dateRange: lessonTime === 'future' ? 1 : -1,
    });

    const lessonDate = newLessonDate.toDate();
    scenarioContext.set(aliasLessonDate, lessonDate);
    scenarioContext.set(aliasStartDate, lessonDate);
}
