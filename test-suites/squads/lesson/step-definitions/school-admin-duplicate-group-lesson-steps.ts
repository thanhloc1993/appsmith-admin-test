...
import { LessonStatusType, LessonTimeValueType, MethodSavingType } from '../common/types';
...
Given(
    '{string} has created a {string} {string} group lesson with filled all information in the {string}',
    async function (
        role: AccountRoles,
        ...
        const createLessonTime = parseLessonTime({ lessonTime, methodSaving: savingMethod });

        await cms.instruction(
            `${role} has created a ${lessonStatus} ${savingMethod} group lesson with filled all information in the ${lessonTime}`,
            async function () {
                await createLessonWithGRPC({
                    cms,
                    scenarioContext,
