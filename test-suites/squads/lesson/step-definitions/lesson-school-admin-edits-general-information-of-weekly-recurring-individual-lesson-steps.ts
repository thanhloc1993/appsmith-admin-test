...
import { LessonTimeValueType, LessonUpsertFields } from 'test-suites/squads/lesson/common/types';
...
Given(
    '{string} has created a recurring individual lesson with lesson date in the future and attached pdf and video',
...
Then(
    '{string} sees updated Attendance of Students and not sees {string} for this lesson',
    async function (this: IMasterWorld, role: AccountRoles, material: LessonMaterialSingleType) {
        const cms = getCMSInterfaceByRole(this, role);
        const scenarioContext = this.scenario;
        ...
        await cms.instruction(
            `${role} sees updated Attendance of Students and not sees ${material} for this lesson`,
            async function () {
                await cms.waitingForLoadingIcon();
...
Then(
    '{string} sees other individual lessons in chain no change general information',
    async function (this: IMasterWorld, role: AccountRoles) {
        const cms = getCMSInterfaceByRole(this, role);
        const scenarioContext = this.scenario;

        await cms.instruction(
            `${role} sees other individual lessons in chain no change general information`,
            async function () {
                await assertOtherIndividualLessonsInChainNoChange(cms, scenarioContext);
            }
        );
    }
);
...
Then(
    '{string} sees updated Attendance of Students and {string} Info for this lesson',
    async function (this: IMasterWorld, role: AccountRoles, material: LessonMaterialSingleType) {
        const cms = getCMSInterfaceByRole(this, role);
        const scenarioContext = this.scenario;
        ...
        await cms.instruction(
            `${role} sees updated Attendance of Students and ${material} Info for this lesson`,
            async function () {
...
Then(
    '{string} sees updated {string} for this lesson',
    async function (this: IMasterWorld, role: AccountRoles, field: LessonUpsertFields) {
        const cms = getCMSInterfaceByRole(this, role);
        const scenarioContext = this.scenario;
...
Then(
    '{string} sees updated {string} for other individual lessons in chain',
    async function (this: IMasterWorld, role: AccountRoles, field: LessonUpsertFields) {
        const cms = getCMSInterfaceByRole(this, role);
        const scenarioContext = this.scenario;
