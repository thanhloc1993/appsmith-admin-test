...
import {
    ...
    RadioButtonLessonTeachingMediumOffline,
...
export async function changeLessonDateToTomorrowV2(cms: CMSInterface, dateRange = 1) {
...
export async function selectDateAndTimeOfFutureV2(cms: CMSInterface) {
    await changeLessonDateToTomorrowV2(cms);
...
export async function checkTeachingMedium(cms: CMSInterface, type: 'online' | 'offline') {
    switch (type) {
        case 'offline':
            await cms.instruction(`Filled ${type} teaching medium`, async function () {
                await cms.page!.check(RadioButtonLessonTeachingMediumOffline);
            });
            break;

        case 'online':
            await cms.instruction(`Filled ${type} teaching medium`, async function () {
                await cms.page!.check(RadioButtonLessonTeachingMediumOnline);
            });
            break;
