...
import { lessonTimeTableWithNth } from '../common/cms-selectors';
...
export async function assertLessonEndDateInChains(props: AssertLessonEndDateRecurringChains) {
    ...
    for (let i = 0; i < indexOfChains.length; i++) {
        ...
        await selectLessonByIndex({ cms, order: indexOfChains[i], scenarioContext, lessonTime });
...
export async function selectLessonByIndex(props: SelectLessonByIndex) {
    ...
    const theLesson = await cms.page!.waitForSelector(lessonLinkByOrderWithTab(order, lessonTime));

    const data = await cms.page!.getAttribute(
        lessonLinkByOrderWithTab(order, lessonTime),
        'data-value'
    );
