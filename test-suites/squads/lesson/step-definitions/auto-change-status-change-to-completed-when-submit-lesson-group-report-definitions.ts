...
export async function assertLessonStatus(cms: CMSInterface, lessonStatusType: LessonStatusType) {
    await cms.instruction(`Assert lesson status in lesson detail page on CMS`, async function () {
        const lessonStatus = await cms.page!.locator(tableLessonStatus).innerText();
        weExpect(lessonStatus, `Lesson status to equal ${lessonStatusType}`).toEqual(
            lessonStatusType
        );
    });
}
