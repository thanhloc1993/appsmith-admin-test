...
import {
    ...
    lessonLinkByOrder,
    lessonLinkOfRowOnLessonListByOrder,
    lessonTableRow,
...
export async function goToLessonDetailByMiddleLesson(params: {
    ...
    await cms.instruction('Go to the lesson detail', async function () {
        ...
        await selectLessonByOrder({ cms, scenarioContext: scenario, order: orderMiddleLessonList });
    });
...
export async function selectLessonByOrder(props: SelectLessonByOrder) {
    ...
    const theLesson = await cms.page!.waitForSelector(lessonLinkOfRowOnLessonListByOrder(order));

    const data = await cms.page!.getAttribute(
        lessonLinkOfRowOnLessonListByOrder(order),
        'data-value'
    );
