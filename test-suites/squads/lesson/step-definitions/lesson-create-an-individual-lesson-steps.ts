...
When(
    '{string} applies center location in location settings on Teacher App',
    async function (roles: string) {
        ...
        for (const teacherRole of teacherRoles) {
            const teacher = getTeacherInterfaceFromRole(this, teacherRole);
            await teacher.instruction(
                `${teacherRole} applies center location in location settings on Teacher App`,
                async function () {
                    await teacherOpenLocationFilterDialogOnTeacherApp(teacher);
                    await teacherSelectLocationsOnTeacherApp(teacher, [locationIdFromContext]);
                    await teacherAppliesSelectedLocationOnTeacherApp(teacher);
...
When(
    '{string} clicks save with {string} the lesson page',
    async function (
        this: IMasterWorld,
        role: AccountRoles,
        ...
        await cms.instruction(
            `${role} clicks save with ${lessonActionSave} the lesson page`,
            async function () {
