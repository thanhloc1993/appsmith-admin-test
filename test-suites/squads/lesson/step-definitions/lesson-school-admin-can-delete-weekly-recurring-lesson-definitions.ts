...
import {
    lessonLinkOfSecondRowOnLessonListWithTab,
    lessonDetailDuration,
    lessonOnListWithDataValue,
    lessonLinkOfFirstRowOnLessonListWithTab,
    lessonRowOfSecondRowOnLessonListWithTab,
...
export async function selectLessonLinkByLessonOrder(
    ...
    const theLesson = await cms.page!.waitForSelector(
        lessonLinkOfSecondRowOnLessonListWithTab(lessonTime)
    );

    const data = await cms.page!.getAttribute(
        lessonRowOfSecondRowOnLessonListWithTab(lessonTime),
        'data-value'
    );

    scenario.set(aliasOrderEditLessonWeeklyRecurring, '2nd');
    scenario.set(aliasLessonId, data);
    await theLesson.click();
}

export async function selectLessonFirstLesson(cms: CMSInterface, lessonTime: LessonTimeValueType) {
    const theLesson = await cms.page!.waitForSelector(
        lessonLinkOfFirstRowOnLessonListWithTab(lessonTime)
    );
...
export async function goToLessonDetailByLessonOrderOnLessonList(params: {
    ...
    await cms.instruction('Go to the lesson detail', async function () {
        await selectLessonLinkByLessonOrder(cms, scenario, lessonTime);
    });
    ...
    const lessonRepeatDuration = await cms.page!.locator(lessonDetailDuration).textContent();

    scenario.set(aliasRepeatDuration, lessonRepeatDuration);
}
