...
import { LessonStatusType } from 'test-suites/squads/lesson/common/types';
...
Given(
    '{string} has created a {string} group lesson in the {string} with {string} teaching medium',
    async function (
        role: AccountRoles,
        ...
        await cms.instruction(
            `${role} has created a ${lessonStatus} group lesson in the ${lessonTime} with ${teachingMedium} teaching medium`,
            async function () {
...
When('{string} edits Location of the group lesson', async function (role: AccountRoles) {
    const cms = getCMSInterfaceByRole(this, role);
    ...
    await cms.instruction(`${role} edits Location of the group lesson`, async function () {
