...
import { createLessonEndpoint, updateLessonEndpoint } from '@legacy-step-definitions/endpoints';
...
import { aliasMaterialId, aliasMaterialName } from 'test-suites/squads/lesson/common/alias-keys';
import {
...
export async function selectTeachingMedium(cms: CMSInterface, teachingMedium?: TeachingMedium) {
    ...
    const desireTeachingMedium = teachingMediumRadioButton(teachingMedium || defaultTeachingMedium);
...
export async function saveUpdateLessonOfLessonManagementWithStatus(
    cms: CMSInterface,
    ...
    await cms.instruction(`Click save with ${lessonActionSave} the lesson page`, async function () {
        if (!lessonId) {
...
export async function waitCreateLesson(cms: CMSInterface) {
    return await cms.waitForGRPCResponse(createLessonEndpoint, { timeout: 200000 });
}

export async function waitUpdateLesson(cms: CMSInterface) {
    return await cms.waitForGRPCResponse(updateLessonEndpoint, { timeout: 120000 });
}
...
export async function assertToSeeNewLessonOnLearnerApp(
    ...
    await learner.instruction('Assert lesson is visible', async function () {
        const lesson = new ByValueKey(LearnerKeys.lessonItem(lessonId, ''));
...
export async function submitLessonOfLessonManagementWithMaterials(params: {
    ...
    const { cms, scenarioContext, lessonName, materials, mode } = params;
...
export async function createIndividualLessonOfLessonManagement(params: {
    cms: CMSInterface;
    scenarioContext: ScenarioContext;
    lessonTime?: LessonManagementLessonTime;
    lessonName?: LessonManagementLessonName;
    ...
    const cloneTeachers = [...teacherNames];
    const cloneStudents = [...studentInfos];
    ...
    if (!arrayHasItem(teacherNames)) {
        ...
        cloneTeachers.push(teacherName);
    }
    ...
    if (!arrayHasItem(studentInfos)) {
        ...
        cloneStudents.push(studentInfo);
    }

    // Select lesson date
    if (lessonTime && lessonTime !== 'now') {
        await cms.instruction(`Select date of ${lessonTime}`, async function () {
            if (lessonTime === 'future') await changeLessonDateToTomorrow(cms);
            else await changeLessonDateToYesterday(cms);
        });
    }
    ...
    await cms.instruction('Select time from now to 23:45', async function () {
        await changeTimePicker({
            cms,
            timePickerSelector: endTimeInputV3,
            hour: 23,
            minute: 55,
        });
    });
    ...
    await selectCenterByName(cms, cloneStudents[0].locationName);

    // Add teachers
    for (const teacherName of cloneTeachers) await selectTeacher(cms, teacherName);

    // Add students
    for (const student of cloneStudents) {
        ...
        await retry(
            async function () {
                try {
                    ...
                    const btnCancel = tableAddStudentSubscriptionCancelButton;
                    await cms.page!.click(btnCancel);
                }
            },
            {
                retries: 2,
                delay: 2000,
                until: () => addedStudent === true,
            }
    ...
    if (!materials) await submitLessonOfLessonManagement(cms, scenarioContext, lessonName);
...
export async function submitLessonWithMaterial(params: {
    ...
    const { materials, lessonName, cms, scenarioContext } = params;
    // Submit lesson without materials
    if (!materials) return await submitLessonOfLessonManagement(cms, scenarioContext, lessonName);
