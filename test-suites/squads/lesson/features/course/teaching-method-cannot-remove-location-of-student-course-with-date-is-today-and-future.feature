...
Feature: School admin cannot remove location of a student course with start date is today and end date is future
    Background:
        Given "school admin" logins CMS
        And "school admin" has gone to course page

    Scenario Outline: School admin cannot remove location of a student course with start&end date is today&future by X button
        ...
        And "school admin" has set start date is in the today and end date is in the future
    ...
    Scenario Outline: School admin cannot remove location of a student course with start&end date is today&future by deselecting location
        Given "school admin" has created a new course with "<type>" location
        And "school admin" has created a new student with "<type>" location
        And "school admin" has added course to student with "1" location which is from selected "<type>" location
        And "school admin" has set start date is in the today and end date is in the future
