...
Feature: Can deselect and view parent location
    Background:
    ...
    Scenario Outline: School admin does not see deselected parent location in detail course page
        Given "school admin" has selected parent location in location popup
        And "school admin" has deselected "<type1>" location in location popup
        ...
        Then "school admin" sees location field is blank
        Examples:
            | type1                  |
            | parent                 |
            | all children of parent |